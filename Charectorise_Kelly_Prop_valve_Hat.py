...
file_path = '/home/pi/Charectorise_data.csv'
...
def log_data(time,actual_flow, set_voltage_val, pressure, current_mass, current_gt_mass, sense_current):
    """
    ...
    today = date.today()
    log.write("{0},{1},{2},{3},{4},{5},{6},{7}\n".format(str(today.strftime("%d/%m/%Y")),str(time),str(actual_flow),str(set_voltage_val),str(pressure),str(current_mass),str(current_gt_mass), str(sense_current)))
...
if __name__ == '__main__':
    ...
    # pressure_channel = AnalogIn(adc, MCP.P6)
    # measured_voltage_channel  = AnalogIn(adc, MCP.P7)
    pressure_channel = 0
    sense_resistor_channel = 1
    ...
    select_channel = DAC8532.channel_A
    dac = DAC8532.DAC8532()
    select_value = 0
    ...
    try:
        with open(file_path, "w") as log: 
            ...
            while(True  & (p_after1 < 200.0) & (set_voltage >= 0)): #&  (avg_flow_value>=0.5)):
                ...
                if i >= MV_AVG_DEPTH:
                    ...
                    p_after1 = convert_volt2pressure(adc.ADS1256_GetChannalValue(pressure_channel) * 5.0/0x7fffff) # Full scale output is 0x7fffff for the ADC
                    current_sense = convert_volt2current(adc.ADS1256_GetChannalValue(sense_resistor_channel) * 5.0/0x7fffff)
                    ...
                    dac.DAC8532_Out_Voltage(select_channel, set_voltage)
                    ...
                    # Current_GT_mass = Current_GT_mass + computeMassDelta(avg_pressure_value,p_b efore=0.0, r=287.058, t=293.15)
                    ...
                    time_val = t
                    log_data(time_val,avg_flow_value,set_voltage, avg_pressure_value, Current_mass, Current_GT_mass, current_sense)
                    set_voltage = set_voltage + volt_step
