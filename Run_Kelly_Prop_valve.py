...
file_path = '/home/pi/Inflating_finger_data.csv'
...
def log_data(time,actual_flow,target_flow,pressure, current_mass, current_gt_mass, desired_Mass):
    """
    ...
    today = date.today()
    log.write("{0},{1},{2},{3},{4},{5},{6},{7}\n".format(str(today.strftime("%d/%m/%Y")),str(time),str (actual_flow),str (target_flow),str(pressure),str(current_mass),str(current_gt_mass), str(desired_Mass)))
    ...
    try:
        with open(file_path, "w") as log: 
            ...
            while(True  & (time_val <= TIME_DURATION) & (p_after1 < 200.0) & (Current_mass <= TARGET_MASS)): #&  (avg_flow_value>=0.5)):
                ...
                p_after1 = convert_volt2pressure(pressure_channel.voltage)
                ...
                if i >= MV_AVG_DEPTH:
                    ...
                    time_val = t
                    log_data(time_val,avg_flow_value,targetFlow, avg_pressure_value, Current_mass, Current_GT_mass, desiredMass)
