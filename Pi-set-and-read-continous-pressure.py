...
if os.path.exists('/home/pi/RPi-read-flow-sensor/Voltage_readings_Festo_continous_pressure.csv'):
    os.remove('/home/pi/RPi-read-flow-sensor/Voltage_readings_Festo_continous_pressure.csv')
...
if __name__ == '__main__':
    ...
    step=1
    tolerance = 0.01
    step_voltage = float(input("Please input the voltage step:"))
    try:
        with open("/home/pi/RPi-read-flow-sensor/Voltage_readings_Festo_continous_pressure.csv", "a") as log:
            start = time.time()
            # inflation
            while True and  set_volt<1:                
