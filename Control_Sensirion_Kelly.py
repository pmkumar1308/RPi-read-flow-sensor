...
class ControlFlow:
	...
	def __init__(self, target_value, measured_value, propotional_gain):
		self.target = target_value
		self.measured = measured_value
		self.KP = propotional_gain

	def contolLaw(self, controlVoltage):
		error = self.target - self.measured
		controlVoltage = controlVoltage + error * self.KP
		...
		with open(file_path, "w") as log:  
			while(True & (i<=NumberOfRuns)):				
				...
				if i >= MAX_DEPTH:
					...
					# Control				
					cf = ControlFlow(targetFlow, avg_flow_value, controlKP)
